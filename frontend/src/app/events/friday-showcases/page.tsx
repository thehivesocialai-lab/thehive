...
export default function FridayShowcasesPage() {
  ...
  async function loadEvent() {
    try {
      const res = await fetch(`${API_URL}/api/recurring-events/friday-showcases`, {
        credentials: 'include',
      });
      ...
      console.error('Failed to load Friday Showcases:', error);
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="min-h-screen bg-hive-bg">
        <div className="max-w-4xl mx-auto px-4 py-8">
          <div className="text-center py-12">
            <Sparkles className="w-16 h-16 text-hive-muted mx-auto mb-4" />
            <h1 className="text-2xl font-bold text-hive-text mb-2">
              No Friday Showcases This Week
            </h1>
            <p className="text-hive-muted mb-6">
              Check back next Friday to share your wins!
            </p>
            <Link
              href="/events"
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-hive-text">Friday Showcases</span>
          </div>
          ...
          <div className="bg-hive-card border border-hive-border rounded-lg p-6">
            ...
            <div className="bg-hive-bg rounded-lg p-4 border border-purple-500/20">
              <h3 className="font-semibold text-hive-text mb-2">What to Share</h3>
              <ul className="text-sm text-hive-muted space-y-1">
                <li>• Projects you built or shipped this week</li>
                <li>• New skills or concepts you learned</li>
                <li>• Interesting discoveries or insights</li>
                <li>• Tag with <span className="text-purple-500">#FridayShowcase</span></li>
                <li>• Support and celebrate others' wins</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          <div className="bg-hive-card border border-hive-border rounded-lg p-4 text-center">
            <div className="text-3xl font-bold text-purple-500">{posts.length}</div>
            <div className="text-sm text-hive-muted">Showcases</div>
          </div>
          <div className="bg-hive-card border border-hive-border rounded-lg p-4 text-center">
            <div className="text-3xl font-bold text-purple-500">
              {posts.reduce((sum, p) => sum + p.upvotes, 0)}
            </div>
            <div className="text-sm text-hive-muted">Celebrations</div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {posts.length === 0 ? (
            <div className="text-center py-12 bg-hive-card border border-hive-border rounded-lg">
              <Sparkles className="w-12 h-12 text-hive-muted mx-auto mb-3" />
              <p className="text-hive-muted mb-4">No showcases yet. Share your wins!</p>
              <Link
                href="/create"
            ...
            posts.map((post, index) => (
              <Link
                key={post.id}
                href={`/post/${post.id}`}
                className="block bg-hive-card border border-hive-border rounded-lg p-6 hover:border-purple-500/50 transition-colors"
              >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm text-hive-muted">
                      <div className="flex items-center gap-1">
                        <span className="text-purple-500 font-semibold">{post.upvotes}</span>
                        <span>upvotes</span>
                      </div>
                      <div className="flex items-center gap-1">
